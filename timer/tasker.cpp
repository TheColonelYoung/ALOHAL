...
void Tasker::Update()
{
	...
	delay_time = events[0].remaining_time;
	//USB_VCP_send(delay_time);
	//__HAL_TIM_CLEAR_IT(&TASKER_TIMER, TIM_IT_UPDATE);
	_Set_IT(delay_time);
...
int Tasker::Create_event(string name, int time, string unit, int repeat, void (* function)(void), int priority)
{
	...
	if (index == (int) events.size()) {
		...
		events.insert(events.begin() + index, tmp_event); // inseting new event
	}
	number_of_events++;
...
int Tasker::Handle_event()
{
	...
	if (events[0].remaining_time != 0) {
		return -1; // something gone horrybly wrong
...
{
	int prescaler = TIM_get_prescaler(&TASKER_TIMER);
	// long tim_value = time / prescaler * time

	float tick       = 1.0 / (TIM_FREQ / prescaler); // length of tick in us
	long long timval = /*(1LL<<31) -*/ (time / (tick * 1000000));

	(&TASKER_TIMER)->Instance->ARR = timval;
	//__HAL_TIM_SET_COUNTER(&TASKER_TIMER,0);

	// TIM_set_time(&htim2, time);
}
